 ...
 * @author Alex
 */
public class GoToAp extends ChatCommand
{
	public GoToAp() {
		super("gotoap");
	}
	...
	public void execute(Player player, String... params) 
	{
		...
		{
			PacketSendUtility.sendMessage(player, "syntax //gotoap <location>");
			return;
		}
		...
		if(player.getCommonData().getRace() == Race.ELYOS)
                  ...
                  {
                     goTo(player, WorldMapType.SANCTUM.getId(), 1533, 1512, 570);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь в столицу Элийцев.");
                  }
		 ...
		 */
		// Pandaemonium
		if  (destination.equalsIgnoreCase("Pandaemonium")) {
		if(player.getCommonData().getRace() == Race.ELYOS)
                  {
                     goTo(player, 120010000, 890, 1438,212);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь в столицу Элийцев.");
                  }
                  else if(player.getCommonData().getRace() == Race.ASMODIANS)
                  {
		 ...
		 */
		// Inggison
		if (destination.equalsIgnoreCase("Inggison"))
		 ...
		 */
		    ...
		    else if (destination.equalsIgnoreCase("pvp2"))
		    {
		          if(player.getCommonData().getRace() == Race.ELYOS)
		          {
			         goTo(player, 600010000, 1232, 641, 346);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь на Площадь покоя.");
		          }
		          else if(player.getCommonData().getRace() == Race.ASMODIANS)
		          {
			         goTo(player, 600010000, 1232, 893, 346);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь на Площадь покоя.");
			      }
		    }
            else if (destination.equalsIgnoreCase("pvp3"))
		    {
		          if(player.getCommonData().getRace() == Race.ELYOS)
		          {
			         goTo(player, 600010000, 664, 724, 337);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь на Площадь великанов.");
		          }
		          else if(player.getCommonData().getRace() == Race.ASMODIANS)
		          {
			         goTo(player, 600010000, 758, 816, 338);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь на Площадь великанов.");
			      }
		    }
			...
			else if (destination.equalsIgnoreCase("test1"))
			{
                  ...
                  {
                     goTo(player, 300020000, 104, 86, 25);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь в Test zone I.");
                  }
			}
			else if (destination.equalsIgnoreCase("test2"))
			{
                  ...
                  {
                     goTo(player, 900020000, 144, 156, 20);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь в Test zone II.");
                  }
			}
			else if (destination.equalsIgnoreCase("test3"))
			{
                  ...
                  {
                     goTo(player, 900030000, 228, 191, 49);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь в Test zone III.");
                  }
			}
			else if (destination.equalsIgnoreCase("test4"))
			{
                  ...
                  {
                     goTo(player, 900100000, 196, 167, 20);
                     PacketSendUtility.sendYellowMessageOnCenter(player, "Вы переноситесь в Test zone VI.");
                  }
			}
			...
			else if (destination.equalsIgnoreCase("fan") && player.getAbyssRank().getAp() >= 500000)
			{
			      //AbyssPointsService.addAp(player, -100000);
			      ...
			      PacketSendUtility.sendYellowMessageOnCenter(player, "Необходимо иметь 500 ка для телепорта в эту локацию");
                  return;
			}
		}
	public static void goTo(final Player player, int worldId, float x, float y, float z) {
            if (player.getClientConnection().getAccount().getMembership() == 2 && player.getAbyssRank().getAp() >= 50000/* && !player.isGM()*/)
                {
                   AbyssPointsService.addAp(player, -50000);
                    PacketSendUtility.sendMessage(player, "За телепорт с вас было снято 50 000 АР");
                }
                else if (player.getClientConnection().getAccount().getMembership() != 2/* && !player.isGM()*/) {
                    PacketSendUtility.sendMessage(player, "Эта команда доступна только VIP");
                    return;
                }
                 else if (player.getAbyssRank().getAp() < 50000 /*&& !player.isGM()*/) {
                    PacketSendUtility.sendMessage(player, "У вас не достаточно ап, необходимое кол-во для телепорта 50 000");
                    return;
                }

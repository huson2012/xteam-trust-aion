...
import com.light.gameserver.network.aion.serverpackets.SM_TITLE_INFO;
...
public class Set extends ChatCommand {
	...
	public void execute(Player admin, String... params) {
		...
		if (target == null && !params[0].equals("apall")) {
		...
		else if (params[0].equals("ap")) 
		{
			...
			if (target == admin) {
			    PacketSendUtility.sendMessage(target, "Вам выдано " + ap + " очков.");
			}
			else {
				PacketSendUtility.sendMessage(admin, "Персонажу " + target.getName() + " было выданно " + ap + " очков.");
				PacketSendUtility.sendMessage(target, "Администратор выдал вам " + ap + " очков бездны.");
			}
			message = "Администратор: " + admin.getName() + " выдал персонажу " + target.getName() + " " + ap + " очков бездны.";
        ...
        else if (params[0].equals("aps") && admin.getAccessLevel() == 5)
		{
			int ap;
			try {
				ap = Integer.parseInt(paramValue);
				...
				PacketSendUtility.sendMessage(admin, "Вы ввели неверный параметр!");
				return;
			}
			...
			try {
				...
				return;
                        ...
                        for (Player pp : World.getInstance().getAllPlayers()) {
                            AbyssPointsService.addAp(pp, ap);
                            PacketSendUtility.sendBrightYellowMessageOnCenter(pp, "Щедрый "+admin.getName()+" выдал всем чуток ап "+ap);
                           }

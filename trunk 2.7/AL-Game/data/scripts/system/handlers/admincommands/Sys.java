	...
	public void execute(Player player, String... params) {
		...
		if (params[0].equals("info")) {
			...
			try {
				...
				PacketSendUtility.sendMessage(player, "Сервер будет отключен через " + val + " секунд.");
			}
			...
			try {
				...
				PacketSendUtility.sendMessage(player, "Сервер будет перезагружен через " + val + " секунд.");
			}
			catch (ArrayIndexOutOfBoundsException e) {
				PacketSendUtility.sendMessage(player, "Numbers only!");
			}

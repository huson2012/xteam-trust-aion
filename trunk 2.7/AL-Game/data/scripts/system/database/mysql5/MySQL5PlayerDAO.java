	...
	public void storePlayer(final Player player) {
		...
		try {
			...
			PreparedStatement stmt = con
				.prepareStatement("UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, creation_date=?, last_online=?, quest_expands=?, npc_expands=?, advenced_stigma_slot_size=?, warehouse_size=?, note=?, title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, mentor_flag_time=? WHERE id=?");
			...
			stmt.setTimestamp(12, pcd.getCreationDate());
			stmt.setTimestamp(13, pcd.getLastOnline());
			stmt.setInt(14, player.getQuestExpands());
			stmt.setInt(15, player.getNpcExpands());
			stmt.setInt(16, pcd.getAdvencedStigmaSlotSize());
			stmt.setInt(17, player.getWarehouseSize());

...
public class SM_PLAYER_INFO extends AionServerPacket {
	...
	protected void writeImpl(AionConnection con) {
		...
		if (player.getClientConnection() != null) {
			if (AdminConfig.CUSTOMTAG_ENABLE) {
				switch (player.getClientConnection().getAccount().getAccessLevel()) {
				        case 0:
						nameFormat = AdminConfig.CUSTOMTAG_ACCESS0;
						break;
					case 1:
		...
		if (player.getWorldId() != AdminConfig.PVP_LOCID && player.getWorldId() != AdminConfig.LOC_ID && player.getWorldId() != 600010000 && WeddingService.isMarried(player))
			name += "\ue020" + WeddingService.getWedding(player).getPartnerName();
		
		if (player.getWorldId() == AdminConfig.PVP_LOCID && !WeddingService.isMarriedTo(player, activePlayer) && !player.isGM() || player.getWorldId() == AdminConfig.LOC_ID && !player.isGM())
			name = con.getActivePlayer().getPlayerAccount().getMembership() < 2 ? 
				player.getPlayerClass().getRusname() : player.getName();

		writeS(String.format(nameFormat, name));

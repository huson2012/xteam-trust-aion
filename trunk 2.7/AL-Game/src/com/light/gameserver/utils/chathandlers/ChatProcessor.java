package com.light.gameserver.utils.chathandlers;
...
import com.light.gameserver.configs.main.WeddingsConfig;
...
public class ChatProcessor {
	...
	public boolean handleChatCommand(Player player, String text) {
		if (text.startsWith("//") && player.getAccessLevel() > 0 || text.startsWith("//") && player.getClientConnection().getAccount().getMembership() == 2) 
		{
	...
	private boolean processCommand(Player player, String text, boolean admin) {
		...
		if (cmd == null) {
			if (admin && player.getAccessLevel() > 0)
				PacketSendUtility.sendMessage(player, "[ОШИБКА] Такая команда не найдена //" + alias);
			return true;
		}
		...
		if (!admin && cmd.getLevel() != 0)
			...
			if (player.getAccessLevel() < cmd.getLevel()) {
				...
				if (player.getAccessLevel() > 0)
					PacketSendUtility.sendMessage(player, "[ОШИБКА] Вы должны иметь уровень доступа " + cmd.getLevel() + " для использования команды //" + alias);
				return true;
			}
		}

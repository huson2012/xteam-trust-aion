...
import com.light.gameserver.model.*;
import com.light.gameserver.model.gameobjects.*;
...
import com.light.gameserver.model.stats.container.PlayerGameStats;
import com.light.gameserver.model.team2.group.PlayerFilters.ExcludePlayerFilter;
...
import com.light.gameserver.model.templates.stats.PlayerStatsTemplate;
...
import com.light.gameserver.services.*;
import com.light.gameserver.services.abyss.AbyssPointsService;
import com.light.gameserver.services.abyss.AbyssService;
...
import com.light.gameserver.skillengine.model.*;
import com.light.gameserver.spawnengine.VisibleObjectSpawner;
import com.light.gameserver.taskmanager.tasks.PlayerMoveTaskManager;
import com.light.gameserver.taskmanager.tasks.TeamEffectUpdater;
...
public class PlayerController extends CreatureController<Player> {
    ...
    public void onEnterWorld() {
        ...
        //String fan = "Вы переноситесь в  FAN зону, в точку вашего класса.";
        int ap = AdminConfig.AP_LOC;
        int aps = AdminConfig.AP_LOC2;
        if (getOwner().getWorldId() == AdminConfig.LOC_ID && AdminConfig.LOC_ID != 0) {
            if (getOwner().getAbyssRank().getAp() >= aps) {
                AbyssPointsService.addAp(getOwner(), -ap);
                PacketSendUtility.sendMessage(getOwner(), "За наождение в этой локации с вас было списано "+ap+" АР");
                forALL(getOwner().getName()+" теряет "+ap+" AP и перемещается в //gotoap fan - фан зону");
                if (getOwner().isInGroup2()) {
                    PlayerGroupService.removePlayer(getOwner());
                    PacketSendUtility.sendMessage(getOwner(), "You can not be in the group are in this location");
                }

                getOwner().setAdminEnmity(2);
                if (neutralType > 1) {
                    getOwner().setAdminNeutral(0);
                }
                PacketSendUtility.sendWhiteMessageOnCenter(getOwner(), "Wellcome to enemy location");

                getOwner().clearKnownlist();
                PacketSendUtility.sendPacket(getOwner(), new SM_PLAYER_INFO(getOwner(), false));
                ...
                /*
                 * if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.ASSASSIN) {
                 * TeleportService.teleportTo(getOwner(), 300090000, 196, 290,
                 * 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.CLERIC) { TeleportService.teleportTo(getOwner(),
                 * 300090000, 192, 219, 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.GLADIATOR) {
                 * TeleportService.teleportTo(getOwner(), 300090000, 191, 149,
                 * 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.TEMPLAR) { TeleportService.teleportTo(getOwner(),
                 * 300090000, 263, 118, 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.RANGER) { TeleportService.teleportTo(getOwner(),
                 * 300090000, 332, 147, 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.SORCERER) {
                 * TeleportService.teleportTo(getOwner(), 300090000, 361, 217,
                 * 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.SPIRIT_MASTER) {
                 * TeleportService.teleportTo(getOwner(), 300090000, 333, 285,
                 * 103, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                 * } else if (getOwner().getCommonData().getPlayerClass() ==
                 * PlayerClass.CHANTER) { TeleportService.teleportTo(getOwner(),
                 * 300090000, 263, 342, 105, 0, false);
                 * PacketSendUtility.sendYellowMessageOnCenter(getOwner(), fan);
                }
                 */


            } else if (getOwner().getAbyssRank().getAp() < aps) {
                if (getOwner().getCommonData().getRace() == Race.ELYOS) {
                    TeleportService.teleportTo(getOwner(), WorldMapType.SANCTUM.getId(), 1322, 1511, 568, 0);
                    PacketSendUtility.sendYellowMessageOnCenter(getOwner(), "Вы переноситесь в столицу Элийцев.");
                } else if (getOwner().getCommonData().getRace() == Race.ASMODIANS) {
                    TeleportService.teleportTo(getOwner(), WorldMapType.PANDAEMONIUM.getId(), 1679, 1400, 195, 0);
                    PacketSendUtility.sendYellowMessageOnCenter(getOwner(), "Вы переноситесь в столицу Асмодиан.");
                }
                PacketSendUtility.sendYellowMessageOnCenter(getOwner(), "Необходимо иметь "+aps+" ап для нахождения в этой локации\nВы были отправлены домой.");
                forALL(getOwner().getName()+" покидает фан зону из-за не достатка ап");
                return;
            }
        }
    }
    ...
    public void forALL(String message) {
        for(Player pp : World.getInstance().getAllPlayers()){
            PacketSendUtility.sendMessage(pp, message);
        }
    }
    //TODO [AT] move
    public void onLeaveWorld() {
        InstanceService.onLeaveInstance(getOwner());
        if (getOwner().getWorldId() == AdminConfig.LOC_ID) {
            PacketSendUtility.sendMessage(getOwner(), getOwner().getName() + "!Try....");
            forALL(getOwner().getName()+" покидает фан зону");
        }
        if (getOwner().getAdminEnmity() > 0) {
            getOwner().setAdminEnmity(0);
    ...
    public void cancelCurrentSkill() {
        ...
        if (castingSkill.getSkillMethod() == SkillMethod.CAST) {
            ...
            PacketSendUtility.sendPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), castingSkill.getFirstTarget().getObjectId(), castingSkill.getItemObjectId(),
    ...
    public void cancelActionItemNpc() {
        ...
        if (hasTask(TaskId.ACTION_ITEM_NPC)) {
            ...
            PacketSendUtility.sendPacket(player, new SM_USE_OBJECT(player.getObjectId(), getOwner().getObjectId(), player.getActionItemNpc().getTalkDelay(), player.getActionItemNpc().getEndCondition()));
        }
    }

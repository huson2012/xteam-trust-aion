...
import com.light.gameserver.controllers.attack.AggroInfo;
import com.light.gameserver.controllers.attack.KillList;
...
import com.light.gameserver.utils.stats.StatFunctions;
...
public class PvpService {
	...
	public void doReward(Player victim) {
		...
		if (totalDamage == 0 || winner == null || winner.getRace() == victim.getRace() && winner.getWorldId() != AdminConfig.LOC_ID && victim.getWorldId() != AdminConfig.LOC_ID) {
			return;
		}
		...
		PacketSendUtility.broadcastPacketAndReceive(victim,
			SM_SYSTEM_MESSAGE.STR_MSG_COMBAT_FRIENDLY_DEATH_TO_B(victim.getName(), winner.getName()));
               ...
               for (Player pp : World.getInstance().getAllPlayers()) {
                if (pp.getWorldId() == AdminConfig.LOC_ID && victim.getWorldId() == AdminConfig.LOC_ID) {
                    PacketSendUtility.sendBrightYellowMessageOnCenter(pp,
                            "Персонаж " + winner.getName() + " убивает персонажа " + victim.getName() + "");
                    PacketSendUtility.sendMessage(victim, "Днище!");
                    PacketSendUtility.sendMessage(winner, "Тру...");
                }
               }
               
                  // MessagerAddition.yellowAnnounceToAll("???????? "+victim.getName()+" ???????? ?? ???? "+winner.getName()+ "", 0);
	...
	private boolean rewardPlayer(Player victim, int totalDamage, AggroInfo aggro) {
		...
		// Don't Reward Player of Same Faction.
		if (winner.getRace() == victim.getRace() && winner.getWorldId() != AdminConfig.LOC_ID && victim.getWorldId() != AdminConfig.LOC_ID)
			return false;

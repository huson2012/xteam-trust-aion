...
import com.light.gameserver.network.aion.serverpackets.SM_INVENTORY_INFO;
...
public class ArmsfusionService {
	...
	public static void breakWeapons(Player player, int weaponToBreakUniqueId) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement("UPDATE game.inventory SET fusioned_item = 0 WHERE item_unique_id = " + weaponToBreak.getObjectId());
			stmt.executeUpdate();
			stmt.close();
			con.close();
		} catch(Exception e) {

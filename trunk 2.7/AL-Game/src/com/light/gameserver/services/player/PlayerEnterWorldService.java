...
import com.light.gameserver.GameServer;
import com.light.gameserver.cache.HTMLCache;
import com.light.gameserver.configs.administration.AdminConfig;
import com.light.gameserver.configs.main.*;
import com.light.gameserver.dao.*;
...
import com.light.gameserver.model.account.CharacterPasskey.ConnectType;
...
import com.light.gameserver.services.abyss.AbyssSkillService;
import com.light.gameserver.services.reward.RewardService;
import com.light.gameserver.services.teleport.TeleportService;
import com.light.gameserver.services.toypet.PetService;
import com.light.gameserver.services.transfers.PlayerTransferService;
...
import com.light.gameserver.services.myservice.GuardService;
/**
 * @author ATracer
 */
public final class PlayerEnterWorldService 
{
	...
	//private static final String serverIntro = "Пожалуйста помните:";
	...
	static 
	{
	    ...
	    alBuffer = "Пожалуйста помните:\nРекламы других серверов запрещены!\n======================================\n";
		//alBuffer += "\n";

		if (GSConfig.SERVER_MOTD_DISPLAYREV) 
		{
			alBuffer += "-----------------------------\n";
			alBuffer += "Версия сервера: " + String.format("%-6s", new Version(GameServer.class).getRevision()) + "\n";
		}
		
		alBuffer += "Вы вошли на сервер Light [GM-PVP] 2.7 Full\n";
		alBuffer += "========| Разработчики |=========\n";
		alBuffer += "Alex [Александр] -Skype:[anaele34]\n";
		alBuffer += "Drusik [Андрей] -Skype:[aion-light]\n";
		/* alBuffer += "==========| Skype |============\n";
        alBuffer += "Alex [anaele34]\n";
		alBuffer += "Drusik [aion-light]\n"; */
		alBuffer += "=================================\n"; 
		alBuffer += "По всем вопросам обращатся в скайп,\nлибо на форум: forum.aion-light.ru\n";
		alBuffer += "Наш сайт: <http://aion-light.ru/gm/> \n";
		alBuffer += "Приятной игры на сервере Aion Light.\n";
	...
	public static void enterWorld(AionConnection client, int objectId)
	{
		...
		if (player != null && client.setActivePlayer(player)) 
		{
			...
			PacketSendUtility.sendYellowMessage(player, alInfo);
                        PacketSendUtility.sendYellowMessageOnCenter(player, ""+ GSConfig.SERVER_NAME +": Добро пожаловать " + player.getName() + "! Администрация сервера желает вам приятной и удачной игры :)");
                        if(player.isMarried()) {
                            PacketSendUtility.sendYellowMessage(player, "Ваш(а) супруг(а) ");
                        }
			...
			if (player.isGM()) 
			{
				if (AdminConfig.INVULNERABLE_GM_CONNECTION || AdminConfig.INVISIBLE_GM_CONNECTION
					...
					if (AdminConfig.INVULNERABLE_GM_CONNECTION) 
					{
						...
						PacketSendUtility.sendMessage(player, ">> Вы вошли бессмертным <<");
					}
			...
			GMService.getInstance().onPlayerLogin(player);
                        if (AdminConfig.CUSTOMTAG_PLAYER_ENABLE) {
			   AnnounService.getInstance().onPlayerLogin(player);
                        }
			if (AdminConfig.GUARD_WINDOW){
                           GuardService.getInstance().guardtest(player, client);
			}
			/**
	...
	private static void showPremiumAccountInfo(AionConnection client, Account account) 
	{
		...
		if (membership > 0) 
		{
			...
			client.sendPacket(new SM_MESSAGE(0, null, "Ваш тип аккаунта " + accountType, ChatType.GOLDEN_YELLOW));
		}
	}

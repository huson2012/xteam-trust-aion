...
import com.light.gameserver.model.templates.academy.PvpZoneTemplate;
...
import com.light.gameserver.world.WorldMapInstanceFactory;
...
public class TvtService extends InstanceService {
    ...
    public void initTvt() {
        ...
        for (TvtSchedule.TvtLevel l : tvtSchedule.getTvtLevelList()) {
            for (String tvtTime : l.getTvtTimes()) {
                ...
                log.info("[TvT Event Test] Scheduled tvt of id " + l.getId() + " based on cron expression: " + tvtTime);
            }
        }
        ...
        log.info("[TVT Event Test] Broadcasting Tvt Begining status based on expression: " + TVT_LOOP_STATUS_BROADCAST_SCHEDULE);

    }

    public void startTvt(final int tvtId, final int startTime, final int duration, final int level, final int mapId) {
        log.info("[TvT Event Test] ������ ����������� �� TvT: " + tvtId);
        TvtRegistrator tvt;
        synchronized (this) {
            if (activeTvt.containsKey(tvtId)) {
                log.error("[TvT Event Test] �� ��� ����������������!: " + tvtId);
                return;
            }
        ...
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                 ...
                 * 30 * 1000
                 */);
        log.info("[TvT Event Test] asfa TvtService TvT: " + tvtId);
        tvt.spawnRegNpc();
        sendWelcomeMeaasge();
        timeForBegin(tvt);
    }
    ...
    public boolean regPlayer(Player player) {
        ...
        if (tvt == null) {
            TvtService.sendPublicMsg(player, "[TvT Event Test]Tvt: � ��������� ��� ������� �� ��������!.");
            return false;
        }
    ...
    public void sendWelcomeMeaasge() {
        World.getInstance().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                switch (player.getRace()) {
                    case ASMODIANS:
                    	PacketSendUtility.sendWhiteMessage(player, "[TvT Event Test]Tvt: ������ ������ ������� ��� � ������������.");
                        break;
                    case ELYOS:
                    	PacketSendUtility.sendWhiteMessage(player, "[TvT Event Test]Tvt: ������ ������� ������� ��� � ��������");
                        break;
    ...
    private WorldMapInstance getNextJSUnionInstance(int worldId) {
        ...
        log.info("[TvT Event Test] �������� ������ TvT " + nextInstanceId);
                ...
                World.getInstance().doOnAllPlayers(new Visitor<Player>() {
                    ...
                    public void visit(Player player) {
                        if (!tvt.getHolders().getPlayer(player) && player.getCommonData().getLevel() >= tvt.getLevel()) {
                            switch (player.getRace()) {
                                case ASMODIANS:
                                	PacketSendUtility.sendWhiteMessageOnCenter(player, "[TvT Event Test]Tvt: ����������� ����� ������������� ��: " + tvt.getRemainingTime() / 60 + " minutes.\nAsmodians:"+ tvt.getHolders().getAsmoReg() +" | Elyos:"+tvt.getHolders().getElysReg());
                                    break;
                                case ELYOS:
                                	PacketSendUtility.sendWhiteMessageOnCenter(player, "[TvT Event Test]Tvt: ����������� ����� ������������� ��: " + tvt.getRemainingTime() / 60 + " minutes.\nAsmodians:"+ tvt.getHolders().getAsmoReg() +" | Elyos:"+tvt.getHolders().getElysReg());
                                    break;
